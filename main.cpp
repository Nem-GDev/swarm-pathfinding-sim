...
constexpr float
    OBEDIENCE = 600.f,
    //? Depletion has to be higher than decay for reversed path disappearance
    PHEROMONE_DEPLETION = 400.f,
    PHEROMONE_DECAY = 200.f,
...
int main(int, char **)
{
    ...
    ant1.SetMovementNoisePR(7, MOVEMENT_NOISE, 33);
    ...
    while (window.isOpen())
    {
        ...
        toFood.TickDown(PHEROMONE_DECAY, deltaTime);
        toHome.TickDown(PHEROMONE_DECAY, deltaTime);
        homeSource.UpdateVisualMap();
        foodSource.UpdateVisualMap();
        ...
        antColony.TickMove(MOVE_SPEED, deltaTime);
